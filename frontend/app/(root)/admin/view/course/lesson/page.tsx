...
import Edit from '@/components/admin/view/course/lesson/edit/lesson-edit-admin.component';
import Create from '@/components/admin/view/course/lesson/create/lesson-create-admin.component';
import View from '@/components/admin/view/course/lesson/view/lesson-view-admin.component';
...
const List = () => {
    ...
    const emptyCriteria = new LessonCriteria();
    const service = new LessonAdminService();
    ...
    const {
        ...
        listPremisions,
    } = useListHook<LessonDto, LessonCriteria>({ emptyItem, emptyCriteria,service, t})
    ...
    useEffect(() => {
        activateSecurityConstraint("Lesson")
    ...
    <div className="flex flex-column md:flex-row md:justify-content-between md:align-items-center">
        <h5 className="m-0">{t("lesson.header")}</h5>
        ...
        <div className="col-12">
            <div className="card">
                ...
                <Card title={t("search")} className="mb-5">
                        <div className="grid">
                            <div className="flex flex-column col-3">
                                <label className="mb-1"  htmlFor="1">{t("lesson.modulePlaceHolder")}</label>
                                <Dropdown id="1" value={criteria.module} options={modules} onChange={(e) => setCriteria({ ...criteria, module: e.target.value })} optionLabel="label" filter showClear/>
                            </div>
                            <div className="flex flex-column col-3">
                                <label className="mb-1"  htmlFor="2">{t("lesson.label")}</label>
                                <InputText id="2" value={criteria.label} onChange={(e) => setCriteria({ ...criteria, label: e.target.value })} />
                            </div>
                            <div className="flex flex-column col-3">
                                <label className="mb-1"  htmlFor="3">{t("lesson.description")}</label>
                                <InputText id="3" value={criteria.description} onChange={(e) => setCriteria({ ...criteria, description: e.target.value })} />
                            </div>
                            <div className="flex flex-column col-3">
                                <label className="mb-1"  htmlFor="4">{t("lesson.content")}</label>
                ...
                <DataTable ref={dt} value={items} selection={selectedItems as any} onSelectionChange={(e) => setSelectedItems(e.value as any)} dataKey="id" className="datatable-responsive" filters={globalFilter} header={header} responsiveLayout="scroll" >
                    ...
                    <Column field="module.label" header={t("lesson.module")} sortable ></Column>
                    
                    
                    <Column field="label" header={t("lesson.label")} sortable></Column>
                    
                    
                    <Column field="description" header={t("lesson.description")} sortable></Column>
                ...
                <Dialog visible={deleteItemDialog} style={{width: '450px'}} header={t("confirm")} modal footer={deleteItemDialogFooter} onHide={hideDeleteItemDialog}>
                    ...
                    {item && (<span>{t("lesson.deleteLessonConfirmationMessage")} <b>{item.label}</b>?</span>)}
                    </div>
                </Dialog>
            ...
            <Dialog visible={deleteItemsDialog} style={{ width: '450px' }} header="Confirm" modal footer={deleteItemsDialogFooter} onHide={hideDeleteItemsDialog} >
                <div className="flex align-items-center justify-content-center">
                    <i className="pi pi-exclamation-triangle mr-3" style={{ fontSize: '2rem' }} />
                    {item && <span>{t("lesson.deleteLessonsConfirmationMessage")}</span>}
                </div>
            </Dialog>
