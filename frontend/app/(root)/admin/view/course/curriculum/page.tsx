...
import Edit from '@/components/admin/view/course/curriculum/edit/curriculum-edit-admin.component';
import Create from '@/components/admin/view/course/curriculum/create/curriculum-create-admin.component';
import View from '@/components/admin/view/course/curriculum/view/curriculum-view-admin.component';
...
const List = () => {
    ...
    const emptyCriteria = new CurriculumCriteria();
    const service = new CurriculumAdminService();
    ...
    const {
        ...
        listPremisions,
    } = useListHook<CurriculumDto, CurriculumCriteria>({ emptyItem, emptyCriteria,service, t})
    ...
    useEffect(() => {
        activateSecurityConstraint("Curriculum")
    ...
    <div className="flex flex-column md:flex-row md:justify-content-between md:align-items-center">
        <h5 className="m-0">{t("curriculum.header")}</h5>
        ...
        <div className="col-12">
            <div className="card">
                ...
                <Card title={t("search")} className="mb-5">
                        <div className="grid">
                            <div className="flex flex-column col-3">
                                <label className="mb-1"  htmlFor="1">{t("curriculum.coursePlaceHolder")}</label>
                                <Dropdown id="1" value={criteria.course} options={courses} onChange={(e) => setCriteria({ ...criteria, course: e.target.value })} optionLabel="label" filter showClear/>
                            </div>
                        </div>
                ...
                <DataTable ref={dt} value={items} selection={selectedItems as any} onSelectionChange={(e) => setSelectedItems(e.value as any)} dataKey="id" className="datatable-responsive" filters={globalFilter} header={header} responsiveLayout="scroll" >
                    ...
                    <Column field="course.label" header={t("curriculum.course")} sortable ></Column>
                ...
                <Dialog visible={deleteItemDialog} style={{width: '450px'}} header={t("confirm")} modal footer={deleteItemDialogFooter} onHide={hideDeleteItemDialog}>
                    ...
                    {item && (<span>{t("curriculum.deleteCurriculumConfirmationMessage")} <b>{item.id}</b>?</span>)}
                    </div>
                </Dialog>
            ...
            <Dialog visible={deleteItemsDialog} style={{ width: '450px' }} header="Confirm" modal footer={deleteItemsDialogFooter} onHide={hideDeleteItemsDialog} >
                <div className="flex align-items-center justify-content-center">
                    <i className="pi pi-exclamation-triangle mr-3" style={{ fontSize: '2rem' }} />
                    {item && <span>{t("curriculum.deleteCurriculumsConfirmationMessage")}</span>}
                </div>
            </Dialog>
