...
import Edit from '@/components/admin/view/course/level/edit/level-edit-admin.component';
import Create from '@/components/admin/view/course/level/create/level-create-admin.component';
import View from '@/components/admin/view/course/level/view/level-view-admin.component';
...
const List = () => {
    ...
    const emptyCriteria = new LevelCriteria();
    const service = new LevelAdminService();
    ...
    const {
        ...
        listPremisions,
    } = useListHook<LevelDto, LevelCriteria>({ emptyItem, emptyCriteria,service, t})
    ...
    useEffect(() => {
        activateSecurityConstraint("Level")
        fetchItems(criteria);
    }, []);
    ...
    <div className="flex flex-column md:flex-row md:justify-content-between md:align-items-center">
        <h5 className="m-0">{t("level.header")}</h5>
        ...
        <div className="col-12">
            <div className="card">
                ...
                <DataTable ref={dt} value={items} selection={selectedItems as any} onSelectionChange={(e) => setSelectedItems(e.value as any)} dataKey="id" className="datatable-responsive" filters={globalFilter} header={header} responsiveLayout="scroll" >
                    ...
                    <Column field="code" header={t("level.code")} sortable></Column>
                    
                    
                    <Column field="label" header={t("level.label")} sortable></Column>
                    
                    
                    <Column field="style" header={t("level.style")} sortable></Column>
                ...
                <Dialog visible={deleteItemDialog} style={{width: '450px'}} header={t("confirm")} modal footer={deleteItemDialogFooter} onHide={hideDeleteItemDialog}>
                    ...
                    {item && (<span>{t("level.deleteLevelConfirmationMessage")} <b>{item.label}</b>?</span>)}
                    </div>
                </Dialog>
            ...
            <Dialog visible={deleteItemsDialog} style={{ width: '450px' }} header="Confirm" modal footer={deleteItemsDialogFooter} onHide={hideDeleteItemsDialog} >
                <div className="flex align-items-center justify-content-center">
                    <i className="pi pi-exclamation-triangle mr-3" style={{ fontSize: '2rem' }} />
                    {item && <span>{t("level.deleteLevelsConfirmationMessage")}</span>}
                </div>
            </Dialog>
