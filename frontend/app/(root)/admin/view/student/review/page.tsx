...
import Edit from '@/components/admin/view/student/review/edit/review-edit-admin.component';
import Create from '@/components/admin/view/student/review/create/review-create-admin.component';
import View from '@/components/admin/view/student/review/view/review-view-admin.component';
...
const List = () => {
    ...
    const emptyCriteria = new ReviewCriteria();
    ...
    const {
        ...
        listPremisions,
    } = useListHook<ReviewDto, ReviewCriteria>({ emptyItem, emptyCriteria,service, t})
    ...
    useEffect(() => {
        activateSecurityConstraint("Review")
    ...
    <div className="flex flex-column md:flex-row md:justify-content-between md:align-items-center">
        <h5 className="m-0">{t("review.header")}</h5>
        ...
        <div className="col-12">
            <div className="card">
                ...
                <Card title={t("search")} className="mb-5">
                        <div className="grid">
                            <div className="flex flex-column col-3">
                                <label className="mb-1"  htmlFor="1">{t("review.studentPlaceHolder")}</label>
                            ...
                            <div className="flex flex-column col-3">
                                <label className="mb-1"  htmlFor="2">{t("review.coursePlaceHolder")}</label>
                            ...
                            <div className="flex flex-column col-3">
                                <label className="mb-1"  htmlFor="3-1">{t("review.ratingMin")}</label>
                                <InputNumber id="3-1" value={criteria.ratingMin} onChange={(e) => setCriteria({ ...criteria, ratingMin: e.value })} mode="decimal" />
                            </div>
                            <div className="flex flex-column col-3">
                                <label className="mb-1"  htmlFor="3-2">{t("review.ratingMax")}  </label>
                                <InputNumber id="3-2" value={criteria.ratingMax} onChange={(e) => setCriteria({ ...criteria, ratingMax: e.value })} mode="decimal" />
                            </div>
                            <div className="flex flex-column col-3">
                                <label className="mb-1"  htmlFor="4">{t("review.comment")}</label>
                                <InputText id="4" value={criteria.comment} onChange={(e) => setCriteria({ ...criteria, comment: e.target.value })} />
                            </div>
                            <div className="flex flex-column col-3">
                                <label className="mb-1"  htmlFor="5-1">{t("review.reviewDateMin")}</label>
                                <Calendar id="5-1" value={criteria.reviewDateFrom} onChange={(e) => setCriteria({ ...criteria, reviewDateFrom: e.value as Date })} dateFormat="dd-MM-yy" showIcon={true} />
                            </div>
                            <div className="flex flex-column col-3">
                                <label className="mb-1"  htmlFor="5-2">{t("review.reviewDateMax")}</label>
                                <Calendar id="5-2" value={criteria.reviewDateTo} onChange={(e) => setCriteria({ ...criteria, reviewDateTo: e.value as Date })} dateFormat="dd-MM-yy" showIcon={true} />
                            </div>
                        </div>
                ...
                <DataTable ref={dt} value={items} selection={selectedItems as any} onSelectionChange={(e) => setSelectedItems(e.value as any)} dataKey="id" className="datatable-responsive" filters={globalFilter} header={header} responsiveLayout="scroll" >
                    ...
                    <Column field="student.email" header={t("review.student")} sortable ></Column>
                    
                    
                    <Column field="course.label" header={t("review.course")} sortable ></Column>
                    
                    
                    <Column field="rating" header={t("review.rating")} sortable></Column>
                    
                    
                    <Column field="reviewDate" header={t("review.reviewDate")} sortable body={formateDate("reviewDate")}></Column>
                ...
                <Dialog visible={deleteItemDialog} style={{width: '450px'}} header={t("confirm")} modal footer={deleteItemDialogFooter} onHide={hideDeleteItemDialog}>
                    ...
                    {item && (<span>{t("review.deleteReviewConfirmationMessage")} <b>{item.id}</b>?</span>)}
                    </div>
                </Dialog>
            ...
            <Dialog visible={deleteItemsDialog} style={{ width: '450px' }} header="Confirm" modal footer={deleteItemsDialogFooter} onHide={hideDeleteItemsDialog} >
                <div className="flex align-items-center justify-content-center">
                    <i className="pi pi-exclamation-triangle mr-3" style={{ fontSize: '2rem' }} />
                    {item && <span>{t("review.deleteReviewsConfirmationMessage")}</span>}
                </div>
            </Dialog>
