...
import Edit from '@/components/admin/view/student/enrollment-state/edit/enrollment-state-edit-admin.component';
import Create from '@/components/admin/view/student/enrollment-state/create/enrollment-state-create-admin.component';
import View from '@/components/admin/view/student/enrollment-state/view/enrollment-state-view-admin.component';
...
const List = () => {
    ...
    const emptyCriteria = new EnrollmentStateCriteria();
    const service = new EnrollmentStateAdminService();
    ...
    const {
        ...
        listPremisions,
    } = useListHook<EnrollmentStateDto, EnrollmentStateCriteria>({ emptyItem, emptyCriteria,service, t})
    ...
    useEffect(() => {
        activateSecurityConstraint("EnrollmentState")
        fetchItems(criteria);
    }, []);
    ...
    <div className="flex flex-column md:flex-row md:justify-content-between md:align-items-center">
        <h5 className="m-0">{t("enrollmentState.header")}</h5>
        ...
        <div className="col-12">
            <div className="card">
                ...
                <Card title={t("search")} className="mb-5">
                        <div className="grid">
                            <div className="flex flex-column col-3">
                                <label className="mb-1"  htmlFor="1">{t("enrollmentState.code")}</label>
                                <InputText id="1" value={criteria.code} onChange={(e) => setCriteria({ ...criteria, code: e.target.value })} />
                            </div>
                            <div className="flex flex-column col-3">
                                <label className="mb-1"  htmlFor="2">{t("enrollmentState.label")}</label>
                                <InputText id="2" value={criteria.label} onChange={(e) => setCriteria({ ...criteria, label: e.target.value })} />
                            </div>
                            <div className="flex flex-column col-3">
                                <label className="mb-1"  htmlFor="3">{t("enrollmentState.style")}</label>
                ...
                <DataTable ref={dt} value={items} selection={selectedItems as any} onSelectionChange={(e) => setSelectedItems(e.value as any)} dataKey="id" className="datatable-responsive" filters={globalFilter} header={header} responsiveLayout="scroll" >
                    ...
                    <Column field="code" header={t("enrollmentState.code")} sortable></Column>
                    
                    
                    <Column field="label" header={t("enrollmentState.label")} sortable></Column>
                    
                    
                    <Column field="style" header={t("enrollmentState.style")} sortable></Column>
                ...
                <Dialog visible={deleteItemDialog} style={{width: '450px'}} header={t("confirm")} modal footer={deleteItemDialogFooter} onHide={hideDeleteItemDialog}>
                    ...
                    {item && (<span>{t("enrollmentState.deleteEnrollmentStateConfirmationMessage")} <b>{item.label}</b>?</span>)}
                    </div>
                </Dialog>
            ...
            <Dialog visible={deleteItemsDialog} style={{ width: '450px' }} header="Confirm" modal footer={deleteItemsDialogFooter} onHide={hideDeleteItemsDialog} >
                <div className="flex align-items-center justify-content-center">
                    <i className="pi pi-exclamation-triangle mr-3" style={{ fontSize: '2rem' }} />
                    {item && <span>{t("enrollmentState.deleteEnrollmentStatesConfirmationMessage")}</span>}
                </div>
            </Dialog>
