...
type CurriculumCreateAdminType = {
    visible: boolean,
    onClose: () => void,
    add: (item: CurriculumDto) => void,
    showToast: React.Ref<Toast>,
    list: CurriculumDto[],
    service: CurriculumAdminService,
    t: TFunction
}
const Create: React.FC<CurriculumCreateAdminType> = ({visible, onClose, add, showToast, list, service, t}) => {
    ...
    const emptyItem = new CurriculumDto();
        const {
            ...
            saveItem,
            formateDate
        } = useCreateHook<CurriculumDto, CurriculumCriteria>({list, emptyItem, onClose, add, showToast,service, isFormValid})
    ...
    const onBooleanInputChangeModules = (e: InputSwitchChangeEvent, name: string) => {
       const val = e.value;
        ...
        <TabView activeIndex={activeIndex} onTabChange={onTabChange}>
            <TabPanel header={t("curriculum.tabPan")}>
                <div className="formgrid grid">
                    <div className="field col-6">
                        ...
                        <Dropdown  id="courseDropdown"  value={item.course} options={courses} onChange={(e) => onDropdownChange(e, 'course')}   placeholder={t("curriculum.coursePlaceHolder")} filter filterPlaceholder={t("curriculum.coursePlaceHolderFilter")} optionLabel="label" showClear/>
                    </div>
                </div>
            </TabPanel>
            <TabPanel header={t("curriculum.modules")}>
                    ...
                    <DataTable value={item.modules} tableStyle={{minWidth: '50rem'}} dataKey="id">
                        ...
                        <Button icon="pi pi-pencil" rounded severity="success" className="mr-2" onClick={()=> editModules(rowData)} /> </div>)}></Column>
                    </DataTable>
                    </div>
                    </div>
            </TabPanel>
